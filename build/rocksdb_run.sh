...
sudo $ycsb_root/bin/ycsb run rocksdb -s -P $workload -p recordcount=$count -p rocksdb.dir=/tmp/ycsb-rocksdb-data -p operationcount=$(($count)) -threads 10 | sudo tee $out_path/outputRun.txt
